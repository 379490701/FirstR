...
public class WeatherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {//Android 5.0及以上支持
            //调用getWindow().getDecorView()拿到当前活动的DecorView
            View decorView = getWindow().getDecorView();
            //在调用DecorView的setSystemUiVisibility()方法改变系统UI显示
            //传入的View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN和View.SYSTEM_UI_FLAG_LAYOUT_STABLE表示当前活动的布局会显示在状态栏上面
    ...
    public void requestWeather(final String weatherId) {
        ...
        HttpUtil.sendOkHttpRequest(weatherUrl, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                //然后我们在onResponse()回调中先调用Utility.Utility.handleWeatherResponse()方法将返回的JSON数据转换成Weather对象
                final Weather weather = Utility.handleWeatherResponse(responseText);
                //将线程切回主线程
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        //然后进行判断，如果服务器返回的status状态是ok，说明请求天气成功
    ...
    private void showWeatherInfo(Weather weather) {
        ...
        //在未来几天天气预报部分使用一个for循环来处理每天的天气信息
        //在循环中动态加载forecast.xml布局并设置相应的数据
